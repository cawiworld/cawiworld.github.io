...
function renderLyrics(song) {
    ...
    if (song.lyrics === false || !song.lyrics) {
        ...
        song.lyrics.forEach((l, i) => {
            ...
            let textToDisplay = (isKidsMode && l.kidsText) ? l.kidsText : l.text;
            const lowerText = textToDisplay.trim().toLowerCase();
            
            if (lowerText === '(музыка)' || lowerText === '[music]') {
                p.classList.add('music-line');
                
                let duration = 5;
                if (i < song.lyrics.length - 1) {
                    duration = song.lyrics[i + 1].time - l.time;
                }
...
const searchArtistResult = document.getElementById('search-artist-result');
const saCard = document.getElementById('sa-card');
...
if (openSearchBtn) {
    openSearchBtn.addEventListener('click', () => {
        ...
        gsap.fromTo(searchOverlay, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: 0.3, ease: "power2.out" });
        
        // Очищаем прошлый поиск
        searchInput.value = '';
        ssList.innerHTML = '';
        searchArtistResult.style.display = 'none';
...
if (closeSearchBtn) {
    closeSearchBtn.addEventListener('click', () => {
        gsap.to(searchOverlay, { opacity: 0, scale: 0.95, duration: 0.3, onComplete: () => searchOverlay.style.display = 'none' });
    });
}

const searchInput = document.getElementById('search-input');
const mainContent = document.querySelector('.main-content');
const searchOverlay = document.querySelector('.ap-content') || document.getElementById('search-overlay'); 

const saResult = document.getElementById('search-artist-result');
const saImg = document.getElementById('sa-img');
...
if (searchInput) {
    searchInput.addEventListener('input', (e) => {
        ...
        if(query.length > 0) {
            ...
            if(document.getElementById('search-overlay')) document.getElementById('search-overlay').style.display = 'block';
            else if(searchOverlay) searchOverlay.style.display = 'block';
            
            const matchedArtist = songsData.find(s => s.artist.toLowerCase().includes(query));
            const matchedSongs = songsData.filter(s => s.title.toLowerCase().includes(query) || s.artist.toLowerCase().includes(query));
            
            if (matchedArtist && saResult) {
                saResult.style.display = 'block';
                saImg.src = matchedArtist.cover;
                saName.innerText = matchedArtist.artist;
                saCard.onclick = () => {
                    if(typeof openArtistProfile === 'function') openArtistProfile(matchedArtist.artist);
                };
            } else if (saResult) {
                saResult.style.display = 'none';
            }
            
            if (ssList) {
                ssList.innerHTML = '';
                if (matchedSongs.length > 0) {
                    if (ssTitle) ssTitle.style.display = 'block';
                    ...
                    matchedSongs.forEach(song => {
                        ...
                        item.className = 'ap-song-item';
                        item.innerHTML = `
                            <img src="${song.cover}" style="border-radius: 8px;">
                            <div style="flex-grow: 1;">
                                <h4 style="margin-bottom: 3px;">${song.title}</h4>
                                <span style="font-size: 0.85rem; color: #b2bec3;">${song.artist}</span>
                            </div>
                        `;
                        item.onclick = () => {
                            if(typeof openPlayerView === 'function') openPlayerView(song);
                        };
                        ssList.appendChild(item);
                    });
                } else {
                    if (ssTitle) ssTitle.style.display = 'none';
                    if (!matchedArtist && ssEmpty) ssEmpty.style.display = 'block';
                }
            }
        } else {
            if(mainContent) mainContent.style.display = 'block';
            if(document.getElementById('search-overlay')) document.getElementById('search-overlay').style.display = 'none';
            else if(searchOverlay) searchOverlay.style.display = 'none'; // или как он у тебя изначально скрывался
        }
    });
}